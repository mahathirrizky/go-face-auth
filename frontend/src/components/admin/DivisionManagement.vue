  ...
  <div class="p-6 bg-bg-base min-h-screen">
    ...
    <BaseModal :isOpen="isModalOpen" @close="closeModal" :title="modalTitle" maxWidth="md">
      <form @submit.prevent="saveDivision">
        ...
        />
        <div class="mb-4">
          <label for="description" class="block text-text-muted text-sm font-bold mb-2">Deskripsi:</label>
          <Textarea
            id="description"
            v-model="currentDivision.description"
            rows="3"
            class="w-full"
          />
        </div>
...
const fetchDivisions = async () => {
  ...
  try {
    ...
    divisions.value = response.data.data.map(div => ({
      ...div, // Salin semua properti asli (ID, Name, Shifts, dll.)
      id: div.ID, // Buat properti 'id' (huruf kecil) untuk editKey
      name: div.Name,
      description: div.Description,
      // Pastikan shift_ids dan location_ids selalu ada, bahkan jika kosong
...
const onCellEditComplete = async (event) => {
  ...
  // Langkah 1: Perbarui data lokal secara optimis.
  // Ini adalah langkah krusial yang sebelumnya hilang.
  data[field] = newValue;

  isSaving.value = true;
  try {
    // Langkah 2: Kirim payload hanya dengan data yang diperbarui ke backend.
    const updatePayload = {
      [field]: newValue,
    };
    console.log(`[DEBUG] Saving division ${data.id} with payload:`, JSON.stringify(updatePayload));

    await axios.put(`/api/admin/divisions/${data.id}`, updatePayload);
    toast.add({ severity: 'success', summary: 'Success', detail: 'Divisi berhasil diperbarui!', life: 3000 });
    
    // Perbarui data di array divisions.value secara langsung
    const index = divisions.value.findIndex(d => d.id === data.id);
    if (index !== -1) {
      divisions.value[index] = { ...divisions.value[index], ...updatePayload };
    }
