<template>
  <div class="min-h-screen flex items-center justify-center bg-bg-base py-12 px-4 sm:px-6 lg:px-8">
    <Card class="w-full max-w-md shadow-xl">
      <template #title>
        <h2 class="text-2xl font-bold text-center text-text-base">Reset Kata Sandi Karyawan</h2>
        <p class="mt-2 text-center text-sm text-text-muted">
          Masukkan kata sandi baru Anda.
        </p>
      </template>
      <template #content>
        <Form :resolver="resolver" :initialValues="initialValues" @submit="handleResetPassword" v-slot="{ errors, handleSubmit }">
          <form @submit="handleSubmit" class="p-fluid mt-4">
            ...
            <Button type="submit" label="Reset Kata Sandi" class="w-full mt-4" />
          </form>
        </Form>
      </template>
    </Card>
...
const handleResetPassword = async ({ valid, data }) => {
  ...
  try {
    const response = await axios.post('/api/reset-password', {
      ...
      token_type: 'employee_password_reset'
    });
